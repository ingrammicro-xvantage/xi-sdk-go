# Go API client for xi-sdk-resellers-go
...
```go
import xi-sdk-resellers-go "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `xi-sdk-resellers-go.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), xi-sdk-resellers-go.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `xi-sdk-resellers-go.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), xi-sdk-resellers-go.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `xi-sdk-resellers-go.ContextOperationServerIndices` and `xi-sdk-resellers-go.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), xi-sdk-resellers-go.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), xi-sdk-resellers-go.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), xi-sdk-resellers-go.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, xi-sdk-resellers-go.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
